function [TS_P, TS_T, TS_PET, extra] = GetSingleReplicate_clim(deltaP, deltaT, deltaLowFreqP, deltaSeasonality, HistoricData, info)

	% DK: original function;
	% [TS_Q, TS_P, TS_T, TS_PET, TS_SoilMoisture, extra] = GetSingleReplicate_clim(deltaP, deltaT, deltaLowFreqP, deltaSeasonality, HistoricData, info)
	% [TS_Q, TS_SoilMoisture] from the LHS removed
	% extra_i that goes into [extra] on the LHS removed.
	...
	% (g) Convert T to PET using historic regressions
	% DK: moved down to use if statement.
	
    % (h) Perturb seasonality of rainfall % DK: Moved up one step
    ...
    %[TS_Q, TS_SoilMoisture, extra_i] = GetQ_RunRRmodel(TS_P, TS_PET, info); %DK: not running RR model at all.
	...
	if strcmp(fieldnames(HistoricData), 'PET_monthly') == 1
		...
		extra = CompileAncillaryData(extra_a, extra_b, extra_c, extra_d, extra_e, extra_f, extra_h, ... 
                                 TS_P_ann_HighFreq, TS_P_ann_interim, TS_T_ann, TS_T_ann_interim, HistoricData, extra_g);
								 % extra_i removed
    else
		TS_PET = [];
		extra_g = [];
		extra = CompileAncillaryData(extra_a, extra_b, extra_c, extra_d, extra_e, extra_f, extra_h, ... 
								 TS_P_ann_HighFreq, TS_P_ann_interim, TS_T_ann, TS_T_ann_interim, HistoricData, extra_g);
								 % extra_g switched to NULL (which is related to PET)
	end
...
function extra = CompileAncillaryData(extra_a, extra_b, extra_c, extra_d, extra_e, extra_f, extra_h, TS_P_ann_HighFreq, TS_P_ann_interim, TS_T_ann, TS_T_ann_interim, HistoricData, extra_g)

	% DK: original function;
	% extra = CompileAncillaryData(extra_a, extra_b, extra_c, extra_d, extra_e, extra_f, extra_g, extra_h, extra_i, TS_P_ann_HighFreq, TS_P_ann_interim, TS_T_ann, TS_T_ann_interim, HistoricData)
	% removing extra_i and add if statement for extra_g
