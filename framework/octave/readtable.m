function outtable = readtable(filepath)
    ...
    for iCol = 1:NumCols
		if iCol < 3
			ColName = ThisData.colheaders{iCol}; %DK: numbers themselves cannot be variables names.
		else
			ColName = ['comid_', num2str(ThisData.colheaders{iCol})]; %DK: attaching 'comid_' in front.
		end
