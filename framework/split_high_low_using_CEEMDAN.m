...
function [precip_ann_HighFreq, precip_ann_LowFreq] = AggregateIMFs(HistoricData, LowHighThresh, info)
    ...
    for iSubarea = 1:NumSubareas
		...
		% However, when using clim input short as 15 water years, we only get IMF 1, 2, and residual.
		% This is the temporary work around that consider IMF 1 as high freq, and IMF 2 as low freq... Which is mathematically incorrect.
		% Ultimate solution is using climate data from longer period.
		if LowHighThresh == 1 & (size(this_IMF_set)(1)- LowHighThresh) <= 1
			high = (this_IMF_set(1:LowHighThresh, :));
			low  = (this_IMF_set((LowHighThresh+1):end, :));
		elseif LowHighThresh == 1 & (size(this_IMF_set)(1)- LowHighThresh) > 1
			high = (this_IMF_set(1:LowHighThresh, :));
			low  = sum(this_IMF_set((LowHighThresh+1):end, :));
		elseif LowHighThresh > 1 & (size(this_IMF_set)(1)- LowHighThresh) <= 1
			high = sum(this_IMF_set(1:LowHighThresh, :));
